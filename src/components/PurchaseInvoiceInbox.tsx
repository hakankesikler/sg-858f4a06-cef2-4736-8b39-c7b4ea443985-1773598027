  ...
  return <div className="space-y-5">
    ...
    <Dialog open={Boolean(matchInvoice)} onOpenChange={(open) => !open && setMatchInvoice(null)}><DialogContent className="max-w-5xl max-h-[92vh] overflow-y-auto"><DialogHeader><DialogTitle>Fatura ve İş Eşleştirme</DialogTitle></DialogHeader>{matchInvoice && <div className="space-y-5">
      ...
      {["review_required","match_proposed"].includes(matchInvoice.status) ? <>
        ...
        <label className="flex items-start gap-3 rounded-lg border p-3"><Checkbox checked={checked} onCheckedChange={(value)=>setChecked(Boolean(value))}/><span className="text-sm">Fatura carisini, operasyon taşıyıcısını, KDV hariç matrahı, KDV/tevkifat kırılımını ve seçilen işleri kontrol ettim. Eşleştirmenin doğru olduğunu onaylıyorum.</span></label>
      </> : <>

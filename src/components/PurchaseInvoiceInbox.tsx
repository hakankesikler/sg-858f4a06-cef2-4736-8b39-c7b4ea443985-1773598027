...
const statusLabel: Record<string, string> = {
  ...
  approval_pending: "Fark Kontrolü",
...
export function PurchaseInvoiceInbox() {
  ...
  const matchingTarget = Number(matchInvoice?.net_total || 0);
  ...
  const submitMatch = async () => {
    ...
    try {
      ...
      const description = nextStatus === "payment_pending"
        ? "Sevkiyat, operasyon taşıyıcısı ve fatura carisi eşleştirildi; ödeme kaydı oluşturuldu."
        : nextStatus === "approval_pending"
          ? "Mevcut taşıyıcı veya maliyet farkı ayrıca kontrol edilmelidir."
          : "Fatura iş ile eşleştirildi.";
      toast({ title: nextStatus === "payment_pending" ? "Eşleştirme ve muhasebeleştirme tamamlandı" : "Eşleştirme kaydedildi", description });
      setMatchInvoice(null);
      await load();
    } catch (error: any) {
  ...
  return <div className="space-y-5">
    <div className="grid gap-3 md:grid-cols-5">
      ...
      <Card className="p-4 border-l-4 border-l-amber-500"><div className="text-sm text-slate-500">Fark Kontrolü</div><div className="text-2xl font-bold">{stats.approval_pending}</div></Card>
    ...
    <Dialog open={manualOpen} onOpenChange={setManualOpen}><DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto"><DialogHeader><DialogTitle>Manuel E-Arşiv / Alış Faturası Yükle</DialogTitle></DialogHeader>
      <div className="grid gap-4 md:grid-cols-2">
        ...
        <div className="flex gap-2"><div className="flex-1"><Label>Ödenecek Tutar *</Label><Input type="number" min="0" step="0.01" value={form.grandTotal} onChange={(e) => setForm({...form,grandTotal:e.target.value})}/><p className="mt-1 text-xs text-slate-500">Matrah + KDV − tevkifat</p></div><div className="w-28"><Label>Para Birimi</Label><Select value={form.currency} onValueChange={(value) => setForm({...form,currency:value})}><SelectTrigger><SelectValue/></SelectTrigger><SelectContent>{["TRY","USD","EUR","GBP"].map((item)=><SelectItem key={item} value={item}>{item}</SelectItem>)}</SelectContent></Select></div></div>
    ...
    <Dialog open={Boolean(matchInvoice)} onOpenChange={(open) => !open && setMatchInvoice(null)}><DialogContent className="max-w-5xl max-h-[92vh] overflow-y-auto"><DialogHeader><DialogTitle>Fatura ve İş Eşleştirme</DialogTitle></DialogHeader>{matchInvoice && <div className="space-y-5">
      <div className="grid gap-3 rounded-lg border bg-slate-50 p-4 sm:grid-cols-2 lg:grid-cols-4"><div><div className="text-xs text-slate-500">Fatura</div><div className="font-mono font-semibold">{matchInvoice.invoice_no}</div></div><div><div className="text-xs text-slate-500">Düzenleyen</div><div className="font-semibold">{matchInvoice.issuer_name}</div><div className="text-xs">{matchInvoice.issuer_tax_id}</div></div><div><div className="text-xs text-slate-500">Fatura Carisi</div><div className="font-semibold">{matchInvoice.billing_supplier?.company || matchInvoice.billing_supplier?.name || "Eşleşmedi"}</div></div><div><div className="text-xs text-slate-500">Durum</div><Badge variant="outline" className={statusClass(matchInvoice.status)}>{statusLabel[matchInvoice.status]}</Badge></div><div><div className="text-xs text-slate-500">KDV Hariç Matrah</div><div className="font-semibold">{money(matchInvoice.net_total,matchInvoice.currency)}</div></div><div><div className="text-xs text-slate-500">KDV</div><div className="font-semibold">{money(matchInvoice.vat_total,matchInvoice.currency)}</div></div><div><div className="text-xs text-slate-500">Tevkifat</div><div className="font-semibold">− {money(matchInvoice.withholding_total,matchInvoice.currency)}</div></div><div><div className="text-xs text-slate-500">Ödenecek Fatura Tutarı</div><div className="font-semibold">{money(matchInvoice.grand_total,matchInvoice.currency)}</div></div></div>
      <div className="rounded-lg border border-blue-200 bg-blue-50 p-3 text-sm text-blue-900"><strong>Eşleştirme KDV hariç matrah üzerinden yapılır.</strong> Sevkiyat maliyetlerine KDV ve tevkifat eklenmez. Fatura ödemesi; matrah + KDV − tevkifat kırılımıyla ayrıca izlenir.</div>
      {!matchInvoice.billing_supplier_id && ["review_required","match_proposed","approval_pending","matched"].includes(matchInvoice.status) && <Card className="space-y-3 border-orange-200 bg-orange-50 p-4"><div className="font-semibold text-orange-900">Faturayı düzenleyen cari henüz bağlı değil</div><p className="text-sm text-orange-800">VKN/TCKN ile mevcut cari bulunursa kullanılacak; bulunamazsa eşleştirme sırasında fatura bilgileriyle otomatik oluşturulacaktır.</p>{matchingBillingSuppliers.length > 0 && <div className="flex flex-col gap-2 md:flex-row"><Select value={billingSupplierId || "none"} onValueChange={(value) => setBillingSupplierId(value === "none" ? "" : value)}><SelectTrigger className="flex-1"><SelectValue placeholder="Fatura carisini seçin"/></SelectTrigger><SelectContent><SelectItem value="none">Fatura carisini seçin</SelectItem>{matchingBillingSuppliers.map((supplier) => <SelectItem key={supplier.id} value={supplier.id}>{supplier.company || supplier.name} · {supplier.vergi_no || supplier.tc_no}</SelectItem>)}</SelectContent></Select><Button onClick={() => void assignBillingSupplier()} disabled={busy || !billingSupplierId}>Fatura Carisini Bağla</Button></div>}</Card>}
      {["review_required","match_proposed"].includes(matchInvoice.status) ? <>
        <div><h3 className="font-semibold">Sistemin önerdiği işler</h3><p className="text-sm text-slate-500">Bir veya birden fazla işi seçip her işe düşen KDV hariç maliyeti yazın.</p></div>
        <div className="space-y-2">{candidates.length === 0 ? <div className="rounded-lg border border-amber-200 bg-amber-50 p-4 text-sm text-amber-800"><AlertTriangle className="mr-2 inline h-4 w-4"/>Uygun aday bulunamadı. KDV hariç matrahın tamamını genel gider olarak ayırabilir veya sevkiyat bilgilerini kontrol edebilirsiniz.</div> : candidates.map((candidate) => {
          ...
          return <div key={candidate.shipment_id} className={`grid gap-3 rounded-lg border p-3 md:grid-cols-[32px_1fr_150px_150px] ${state.selected ? "border-blue-300 bg-blue-50" : ""}`}>
            ...
            <div><Label className="text-xs">Gerçekleşen maliyet (KDV hariç)</Label><Input type="number" min="0" step="0.01" disabled={!state.selected} value={state.amount} onChange={(e) => setAllocations({...allocations,[candidate.shipment_id]:{...state,amount:e.target.value}})}/></div>
          </div>;
        })}</div>
        <div className="grid gap-3 md:grid-cols-2"><div><Label>Sevkiyata ait olmayan gider (KDV hariç)</Label><Input type="number" min="0" step="0.01" value={generalExpense} onChange={(e)=>setGeneralExpense(e.target.value)}/></div><div className={`rounded-lg border p-3 ${Math.abs(distributionTotal-matchingTarget)<=0.01 ? "border-emerald-200 bg-emerald-50" : "border-amber-200 bg-amber-50"}`}><div className="text-xs text-slate-500">Dağıtım / KDV Hariç Matrah</div><div className="font-semibold">{money(distributionTotal,matchInvoice.currency)} / {money(matchingTarget,matchInvoice.currency)}</div></div></div>
        ...
        <label className="flex items-start gap-3 rounded-lg border p-3"><Checkbox checked={checked} onCheckedChange={(value)=>setChecked(Boolean(value))}/><span className="text-sm">Fatura carisini, operasyon taşıyıcısını, KDV hariç matrahı, KDV/tevkifat kırılımını ve seçilen işleri kontrol ettim.</span></label>
      </> : <>
        <div><h3 className="font-semibold">Bağlanan işler</h3>{matchInvoice.allocations?.filter((item)=>item.active).map((item)=>{const expected=Number(item.shipment?.cost||0);const difference=Number(item.amount)-expected;return <div key={item.id} className="mt-2 flex justify-between rounded-lg border p-3"><span className="font-mono">{item.shipment?.shipment_code}</span><div className="text-right"><div className="font-semibold">Gerçekleşen (KDV hariç): {money(item.amount,matchInvoice.currency)}</div><div className="text-xs text-slate-500">Tahmini: {money(expected,item.shipment?.cost_currency||matchInvoice.currency)} · Fark: {money(difference,item.shipment?.cost_currency||matchInvoice.currency)}</div></div></div>})}</div>
        {matchInvoice.status === "approval_pending" && <Card className="space-y-3 border-amber-200 bg-amber-50 p-4"><div className="flex items-center gap-2 font-semibold text-amber-900"><AlertTriangle className="h-4 w-4"/>Fark kontrolü gerekiyor</div><p className="text-sm text-amber-800">Sevkiyatta farklı bir operasyon taşıyıcısı bulunuyor olabilir; çoklu iş dağıtımı veya maliyet farkı ayrıca kontrol edilmelidir. Kayıtlı taşıyıcı otomatik değiştirilmez.</p>{matchInvoice.operational_supplier_id && <><Label>Fatura carisi ile operasyon taşıyıcısı ilişkisinin açıklaması</Label><Textarea value={issuerReason} onChange={(e)=>setIssuerReason(e.target.value)} placeholder="Örn. operasyon taşıyıcısının onaylı alt yüklenicisi veya fatura düzenleyicisidir."/><Button variant="outline" onClick={() => void approveIssuer()} disabled={busy}><Link2 className="mr-2 h-4 w-4"/>Taşıyıcı–Fatura Carisi İlişkisini Onayla</Button></>}</Card>}
        {["matched","approval_pending"].includes(matchInvoice.status) && <Card className="space-y-3 border-emerald-200 bg-emerald-50 p-4"><div className="flex items-center gap-2 font-semibold text-emerald-900"><CheckCircle2 className="h-4 w-4"/>Muhasebeleştirmeyi tamamla</div><Textarea value={approvalNote} onChange={(e)=>setApprovalNote(e.target.value)} placeholder="Kontrol notu (isteğe bağlı)"/><Button onClick={() => void approveInvoice()} disabled={busy}>{matchInvoice.status === "approval_pending" ? "Farkı Onayla ve Muhasebeleştir" : "Muhasebeleştir"}</Button></Card>}
      </>}
    </div>}<DialogFooter>{matchInvoice && ["review_required","match_proposed"].includes(matchInvoice.status) && <Button onClick={() => void submitMatch()} disabled={busy || !checked || Math.abs(distributionTotal-matchingTarget)>0.01}>{busy && <Loader2 className="mr-2 h-4 w-4 animate-spin"/>}Eşleştir ve Muhasebeleştir</Button>}</DialogFooter></DialogContent></Dialog>
  </div>;
}

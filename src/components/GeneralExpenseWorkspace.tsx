  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
        <div><h3 className="text-xl font-bold">Genel Gider Yönetimi</h3><p className="text-sm text-slate-500">Giderleri, açık bakiyeleri ve kategorileri tek ekrandan izleyin.</p></div>
      ...
      <Tabs defaultValue="records">
        ...
        <TabsContent value="records" className="mt-4 space-y-3">
          ...
          <Card className="overflow-hidden"><div className="overflow-x-auto"><Table><TableHeader><TableRow>
            ...
            {filteredExpenses.length === 0 ? <TableRow><TableCell colSpan={9} className="py-12 text-center text-slate-500">Henüz genel gider bulunmuyor.</TableCell></TableRow> : filteredExpenses.map((row) => {
        ...
        <TabsContent value="catalog" className="mt-4 space-y-4">
          <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between"><div><h4 className="font-semibold">Gider sınıflandırması</h4><p className="text-sm text-slate-500">Kategorileri ve kullanılacak gider tiplerini yönetin.</p></div>{canManage && <div className="flex flex-wrap gap-2"><Button variant="outline" onClick={() => setCategoryDialog(true)}><Plus className="mr-2 h-4 w-4"/>Kategori Ekle</Button><Button onClick={() => { setNewTypeCategory(categories.find((row) => row.is_active !== false)?.id || ""); setTypeDialog(true); }}><Plus className="mr-2 h-4 w-4"/>Gider Tipi Ekle</Button></div>}</div>

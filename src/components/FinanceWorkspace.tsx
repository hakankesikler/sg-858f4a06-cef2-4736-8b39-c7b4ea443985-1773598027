...
export function FinanceWorkspace({ accounts, transactions, canSync = false, syncing = false, onSync }: FinanceWorkspaceProps) {
  ...
  const exportTransactions = async () => {
    await downloadExcel(`rex-finans-hareketleri-${new Date().toISOString().slice(0, 10)}.xlsx`, filtered.map((row) => ({
      ...
      "Kümülatif Bakiye": Number(row.cumulative_balance || 0), Kaynak: row.source === "kolaybi" ? "Muhasebe" : "REX TYS",
    })), "Finans Hareketleri");
  };

  return <div className="space-y-5">
    <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
      <div><h3 className="text-xl font-bold">Banka, Kasa ve Hesap Hareketleri</h3><p className="text-sm text-slate-500">Güncel bakiyeler, tahsilat, ödeme ve virman kayıtları tek ekranda.</p></div>
    ...
    <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
      {accounts.map((row) => <Card key={row.id}><CardContent className="p-5"><div className="flex items-center justify-between"><Landmark className="h-5 w-5 text-[#173f73]" /><Badge variant="outline">{row.currency || "TRY"}</Badge></div><p className="mt-3 font-semibold">{row.account_name}</p><p className="mt-2 text-2xl font-bold">{amount(row.provider_balance ?? row.balance, row.currency)}</p><p className="mt-1 text-xs text-slate-500">{row.bank_name || row.account_type}{row.last_synced_at ? ` · ${new Date(row.last_synced_at).toLocaleString("tr-TR")}` : ""}</p></CardContent></Card>)}
      {accounts.length === 0 && <Card className="md:col-span-2 xl:col-span-3"><CardContent className="p-8 text-center text-slate-500">Henüz finans hesabı bulunmuyor.</CardContent></Card>}
    </div>

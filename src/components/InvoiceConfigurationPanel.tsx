      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          ...
          {canManage && <div className="space-y-3 rounded-xl border border-orange-200 bg-orange-50/40 p-4">
            ...
            <div><Label>Kalem açıklama şablonu *</Label><Textarea rows={2} value={templateForm.line_description_template} onChange={(e) => setTemplateForm({ ...templateForm, line_description_template: e.target.value })}/><p className="mt-1 text-xs text-slate-500">Kullanılabilir alanlar: {"{{shipment_code}}"}, {"{{origin}}"}, {"{{destination}}"}, {"{{tracking_number}}"}, {"{{service_type}}"}</p></div>

...
export type KolayBiAssociateSyncResult = {
  success: true;
...
export async function synchronizeKolayBiAssociate(input: {
  admin: SupabaseClient<any>;
  customerId: string;
  actorId?: string | null;
  actorEmail?: string | null;
}): Promise<KolayBiAssociateSyncResult> {
  const { admin, customerId, actorId = null, actorEmail = null } = input;
  ...
  if (![10, 11].includes(identityNo.length)) throw reviewError("KolayBi aktarımı için 10 haneli VKN veya 11 haneli TCKN zorunludur.");
  ...
  if (Number.isSafeInteger(storedContactId) && storedContactId > 0) {
    try {
      ...
      // Eski veya geçersiz kimlik kesin VKN/TCKN aramasıyla onarılır.
    }
  }
  ...
  if (!associate) {
    ...
    if (exactMatches.length > 1) {
      throw reviewError("KolayBi aynı VKN/TCKN için birden fazla cari döndürdü. Yanlış eşleşmeyi önlemek için Entegrasyon Merkezi'nden kontrol edin.");
    }
  ...
  if (conflictingMapping?.local_entity_id) {
    throw reviewError(`Bu KolayBi carisi başka bir REX TYS kartıyla eşleşmiş: ${conflictingMapping.display_name || "kayıt"}.`);
  }
  ...
  }).eq("provider_environment", providerEnvironment).eq("resource_type", "associate")
  ...
  const { error: mappingError } = await admin.from("kolaybi_master_records").upsert({
    ...
    external_code: associate?.code || customer.customer_code,
    tax_identity: identityNo,
    payload: { id: contactId, code: associate?.code, name: associate?.name, surname: associate?.surname, identity_no: identityNo, address: associate?.address || associate?.addresses || [] },
    last_seen_at: now,
  ...
  await admin.from("kolaybi_sync_events").insert({
    ...
    status: "success",
    summary: createdInProvider
      ? `${customer.company || customer.name} KolayBi ${providerEnvironment === "live" ? "canlı" : "sandbox"} ortamında otomatik oluşturuldu ve eşleştirildi`
      : `${customer.company || customer.name} VKN/TCKN ile KolayBi'ye otomatik eşleştirildi`,
    actor_id: actorId,
    actor_email: actorEmail,
    metadata: { customer_id: customerId, address_id: addressId, automatic: true, match_method: matchMethod },
  });

  // Anlık form senkronizasyonu başarılıysa aynı kaydın kuyrukta tekrar çalışmasını önler.
  await admin.from("kolaybi_outbound_jobs").update({
    status: "completed", locked_at: null, locked_by: null, last_error: null, updated_at: now,
  }).eq("resource_type", "associate").eq("entity_id", customerId);

  return {
    success: true,
    alreadyLinked: matchMethod === "existing_reference",
    created: createdInProvider,
    matched: !createdInProvider,
    matchMethod,
